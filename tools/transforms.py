...
# 下面把所有可能需要的变化列出来
# 缩放 Scale(scale) 这里scale是最长边/最短边需要到达的值（这里最长边还是最短取决于largest参数）
class Scale(object):
    ...
    def get_params(self, imsize):
        ...
        if isinstance(self.size, int):
            ...
            # 已经达到缩放的比例
            if (cmp(w, h) and w == self.size) or (
                    cmp(h, w) and h == self.size):
                ow, oh = w, h
            # 未达到缩放比例
            elif cmp(w, h):
                ow = self.size
...
class RandomScale (Scale):
    def __init__(self, min_size, max_size, ar=1,
        ...
        """
        Scale.__init__(self, size=0, can_upscale=can_upscale, can_downscale=can_downscale, interpolation=interpolation)
    ...
    def get_params(self, imsize):
        w, h = imsize
        # 如果min_size max_size是缩放因子，要将其转化为具体的值
        min_size = 0
        max_size = 0
...
class RandomRotation(object):
    ...
    def __call__(self, inp):
        ...
        # 这里要先将原点挪中心，这才是按中心旋转的矩阵，后面旋转之后还原
...
# 随机加入噪点，先定义父类，这个父类不会对新的图片做任何的persp的变化
